...
export default function PrintVisibilitySettings() {
  const { userData, dispatch, goBack } = useApp();
  const vis = userData?.fieldVisibility?.print || {};

  const handleToggle = (key, val) => {
    dispatch({ type: 'SET_FIELD_VISIBILITY', visType: 'print', key, value: val });
  };

  const handleToggleAll = (keys, val) => {
    keys.forEach(key => dispatch({ type: 'SET_FIELD_VISIBILITY', visType: 'print', key, value: val }));
  };
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--surface-2)', animation: 'slideInRight 0.3s ease forwards' }}>
      ...
      <div className="screen-scroll">
        <div className="hint-box" style={{ margin: '12px 16px 0', fontSize: 11 }}>
          🖨️ These settings are <strong>independent</strong> of Break-Up Settings. Only ticked fields appear on the printed slip. Discounts are handled separately at print time with opt-in checkboxes.
        </div>
                ...
                <div key={f.key} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{
                      ...
                      background: vis[f.key] ? '#7B3FE0' : 'var(--neutral-300)',
                  ...
                  <label className="toggle-switch">
                    ...
                    />
                    <div className="toggle-track" style={{ '--toggle-on': '#7B3FE0' }}>
